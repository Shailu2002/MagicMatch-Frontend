...
export const Religion = () => {
  ...
  if(res2.status === 422 || !deletedata){
    ...
    const [getReligion, setReligion] = useState({ name: "" })
    const setdata = (e) => {
    ...
    const addinpdata = async (e) => {
        ...
        if (!name) {
            ...
            if (!data || res.status === 404) {
                ...
                toast.success("Religion is added");
            }
        }
    }
  ...
  const handlePrevious1 = () => {
    ...
    useEffect(() => {
      const pagedataCount1 = Math.ceil(religion.length / 8);
      setPageCount1(pagedataCount1)
      if (page1) {
        const LIMIT1 = 8;
        const skip1 = LIMIT1 * page1;
        const dataskip1 = religion.slice(page1 === 1 ? 0 : skip1 - LIMIT1, skip1);
            ...
            <div class="w3-card-4 mt-5 ms-5" style={{ "width": "450px", "height": "188px" }}>
                ...
                <form method="post">
                    <div class="w3-container w3-white">
                        <div class="mb-3">
                            <label class="form-label ms-2 mt-2">Religion</label>
                            <input type="text" class="form-control ms-2 mt-2" name="name" value={getReligion.name} onChange={setdata} placeholder="" style={{ "width": "100px" }} />
                        </div>
                    </div>
