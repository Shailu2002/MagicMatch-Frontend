...
export const Language = () => {
  ...
  const [getLanguage,setLanguage]=useState({name:""})
  const setdata=(e)=>{
  ...
  const addinpdata=async(e)=>{
      e.preventDefault();
      const {name}=getLanguage
      if (!name)
      {
         toast.error("enter Language name");
  ...
  const handlePrevious1 = () => {
    ...
    useEffect(() => {
      const pagedataCount1 = Math.ceil(languagelist.length / 8);
      setPageCount1(pagedataCount1)
      if (page1) {
        const LIMIT1 = 8;
        const skip1 = LIMIT1 * page1;
        const dataskip1 = languagelist.slice(page1 === 1 ? 0 : skip1 - LIMIT1, skip1);
  ...
  return (
   ...
   <div className='row'>
    <div className='col'>
<div class="w3-card-4 mt-5 ms-5" style={{ "width": "450px", "height": "188px" }}>
  ...
  <label  class="form-label ms-2 mt-2">Language</label>
  <input type="text" class="form-control ms-2 mt-2" name="name" value={getLanguage.name} onChange={setdata} placeholder="" style={{"width":"100px"}}/>
</div>
          </div>
