...
const [citylist, setcityList] = useState([]);
  ...
  const handleCountry = (event) => {
    ...
    const countrycodes = countrynames.map((element) => {
 ...
 const handleStateChange = (event) =>
 {
  ...
  const marital = ["Never Married","Divorced","Awaiting Divorce","Widowed",'open to all'];
  const diet = ['Vegeterian','Non-Vegeterian','Occasionally Non-Vegeterian','open to all'];
  const highest_qualification = ['MTech', 'Msc', 'MCA', 'MBA', 'Mcom', "Btech",'MBBS', 'BCA', 'Bcom', 'Bsc', 'Bvoc', 'BBA', 'Intermediate', 'High School', 'Less than School','open to all'];
        ...
        else
        {
          const { user_id,partner_gender, partner_min_age, partner_max_age, partner_min_height, partner_max_height,
            ...
            if (sign.status === 404 || !resp) {
              ...
              history("/user_photo", { state: {gent:id,gender:gender,name:location.state.name},replace:"true" });
            }
          }
      }//onsubmit end here
        ...
        <div className='container cardstyle'>
          <div className='card'>
            <div className='card-body card-mt'>
              ...
              <h4 className='text-center heading1 text-danger'><i class="fa-solid text-dark fa-circle-plus"></i> Partner Preferences</h4>
                <div className='mb-3'>
       ...
       <Select onChange={formik.handleChange} onBlur={formik.handleBlur}
         ...
         { 
           ...
           <MenuItem key={element.name} value={element.name}>
             <Checkbox checked={formik.values.partner_religion.indexOf(element.name) > -1} />
 ...
 { formik.touched.partner_diet && formik.errors.partner_diet ? (<div className='text-danger mb-3'>{formik.errors.partner_diet}</div>) : null} 
       ...
       <Select onChange={formik.handleChange} onBlur={formik.handleBlur}
         ...
         { mother_tongue.map((element,id) => (
           <MenuItem key={id} value={element.name}>
                 ...
                 MenuProps={MenuProps} >
                          {
                           ...
                           <MenuItem key={element.name} value={element.name}>
                           <Checkbox checked={partner_state.indexOf(element.name) > -1} />
                           <ListItemText primary={element.name} />
                           </MenuItem>
         ))}
       </Select>
                   ...
                   <label>City</label>
                     ...
                     value={partner_city} multiple
                          ...
                          renderValue={(selected) => selected.join(', ')}
                 MenuProps={MenuProps} >
         { citylist.map((element,id) => (
           <MenuItem key={element.name} value={element.name}>
             <Checkbox checked={partner_city.indexOf(element.name) > -1} />
             <ListItemText primary={element.name} />
           </MenuItem>
         ))}
       </Select>
