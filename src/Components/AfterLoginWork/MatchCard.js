...
const MatchCard = ({ users,iduser }) => { 
    ...
    const getdata = async () =>
    {
        ...
        if (!data || res.status === 404) {
            ...
            {
              //match email
                emailjs.sendForm('service_s7bc7so', 'template_5r34th9', form.current, '7apsjFDK6tP05LZc-')
                    .then((result) => {
  ...
  return (
      <>
          {users.length === 0 ? <div className=' text-center text-white heading1'>No Matches </div> :
              (users.length === 1 ? <div className=' text-center text-white heading1'> You have 1 Match </div> :
                  (<div className='heading1 text-white text-center'> <span className='text-dark'>You</span> have <span className='text-danger'> {users.length} </span>  <span>Matches.. </span> </div>))}
          {
         pagedata.map((element,id) => {  
            return(
             <>  
              <div style={{paddingTop:"2%"}} className='container'>
                <div style={{ width: "70%", marginLeft: "15%" ,marginBottom:"10px",borderRadius:"28px"}} className='card'>
                            ...
                            <div className='container'>
                                <div className='row'>
                                    ...
                                    <div  className='col-sm-2 text-center'>  
                                        ...
                                        {
                                            iduser.Payment.length === 0 ? <div><button onClick={sendpayment} style={{ border: "none", backgroundColor: "white" }}><i style={{ fontSize: "50px", color: "green" }} class="fa-solid fa-circle-check"></i></button> <p>Upgrade to  Connect</p></div> :
                                            ((iduser.Payment[0].approval_status === 0 || iduser.Payment[0].approval_status === -1) ?
 ...
 <form ref={form}>
 <input hidden type="email" value={ localStorage.getItem("user_email")} name="user_email" /> 
 <input hidden type="text" value={localStorage.getItem("name")}    name="user_name"/>                                    
 ...
 <input hidden type="text" name="message" value="" />                                          
          ...
          {
          pagedata.length !=0 ?  <div className='d-flex  justify-content-center   mt-12 '>
          <Pagination >
